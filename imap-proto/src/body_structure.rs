...
named!(
    body_lang<Option<Vec<&str>>>,
    alt!(
        map!(nstring_utf8, |v| v.map(|s| vec![s]))
            | delimited!(
                char!('('),
                map!(
                    separated_nonempty_list!(opt!(space), string_utf8),
                    Option::from
                ),
                char!(')')
            )
...
named!(
    body_param<Option<Vec<BodyParam>>>,
    alt!(
        map!(nil, |_| None)
            | delimited!(
                char!('('),
...
named!(
    body_disposition<Option<BodyDisposition>>,
    alt!(
        map!(nil, |_| None)
            | delimited!(
                char!('('),
...
named!(
    body_type_message<BodyStructure>,
    do_parse!(
